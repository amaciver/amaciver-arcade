...
│    ├── Resolve channel name → ID (conversations.list)                  │
│    ├── Ensure bot membership (conversations.join)                      │
...
Auth Resolution (CLI Agent):
  Default mode:
    1. Session cache
    2. Arcade OAuth (ARCADE_API_KEY → browser → token with chat:write, im:write, users:read)
       ⚠ NO files:write → images saved locally (file path output + ASCII preview), text-only DMs

  --slack mode:
    1. Session cache
    2. SLACK_BOT_TOKEN (requires all scopes: chat:write, im:write, users:read, files:write, channels:read, channels:join)
       ✓ Full features: uploads images directly to Slack channels
```
...
The CLI agent supports two auth modes:

**Default mode** (no flag):
1. **Session cache** - reuses token within a session
2. **Arcade OAuth** (`ARCADE_API_KEY`) - opens browser for Slack authorization. Supports `chat:write`, `im:write`, `users:read`. **Does NOT support `files:write`**, so image uploads to Slack are unavailable; instead, images are saved locally (agent outputs file path), shown as ASCII art preview, and text-only fact is sent to DM.

**`--slack` mode** (explicit opt-in):
1. **Session cache** - reuses token within a session
2. **`SLACK_BOT_TOKEN` env var** - direct token, supports all features including `files:write` for uploading images directly to Slack channels
...
│   ├── test_agent.py      # System prompt, demo, tool wrappers, auth, Arcade OAuth (46 tests)
